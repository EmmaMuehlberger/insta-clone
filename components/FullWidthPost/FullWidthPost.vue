<template>
	<div class="FullWidthPostOverlay" @click="goToPrev()">
		<svg class="FullWidthPostOverlay__close" aria-label="Close" color="#ffffff" fill="#ffffff" height="24" role="img" viewBox="0 0 24 24" width="24"><polyline fill="none" points="20.643 3.357 12 12 3.353 20.647" stroke="#ffffff" stroke-linecap="round" stroke-linejoin="round" stroke-width="3"></polyline><line fill="none" stroke="#ffffff" stroke-linecap="round" stroke-linejoin="round" stroke-width="3" x1="20.649" x2="3.354" y1="20.649" y2="3.354"></line></svg>
		<div class="FullWidthPost" @click.stop>
			<div class="FullWidthPost__img">
				<img :src="require(`@/assets/img/${post.img[currentImgIndex]}`)" alt="post">
				<div v-if="post.img.length > 1 && currentImgIndex > 0" class="FullWidthPost__img__arrow FullWidthPost__img__arrow--left" @click="prevImg"></div>
				<div v-if="post.img.length > 1 && currentImgIndex < post.img.length - 1" class="FullWidthPost__img__arrow FullWidthPost__img__arrow--right" @click="nextImg"></div>
			</div>
			<div class="FullWidthPost__content">
				...
				<div class="FullWidthPost__content__footer">
					<div class="FullWidthPost__content__footer__icons">
						<!-- Like -->
						...
						<!-- Don't like anymore -->
						<svg v-else aria-label="Unlike" class="FullWidthPost__content__footer__icons__likeIcon" color="#ed4956" fill="#ed4956" height="24" role="img" viewBox="0 0 48 48" width="24" @click="unlikeThisPost"><path d="M34.6 3.1c-4.5 0-7.9 1.8-10.6 5.6-2.7-3.7-6.1-5.5-10.6-5.5C6 3.1 0 9.6 0 17.6c0 7.3 5.4 12 10.6 16.5.6.5 1.3 1.1 1.9 1.7l2.3 2c4.4 3.9 6.6 5.9 7.6 6.5.5.3 1.1.5 1.6.5s1.1-.2 1.6-.5c1-.6 2.8-2.2 7.8-6.8l2-1.8c.7-.6 1.3-1.2 2-1.7C42.7 29.6 48 25 48 17.6c0-8-6-14.5-13.4-14.5z"></path></svg>
						<!-- Comment -->
...
export default {
	...
	created() {
		this.post = this.$store.getters.getSinglePost(this.postId)[0];
		this.user = this.$store.getters.getSingleUser(this.post.user)[0];
	...
	methods: {
		...
		followUser() {
			this.$store.commit("startFollowing", Number(this.user.id));
		},
		unfollowUser() {
			this.$store.commit("stopFollowing", Number(this.user.id));
		},
		getUser(id) {
...
<style scoped lang="scss" src="./FullWidthPost.scss"></style>